...
function BFS() {
    ...
    while(needVisit.length!==0) {
        ...
        if(A[p][q]!==0) { //이미 갈 곳엔 거리가 할당되어있음
            for(let j=0; j<4; j++) { //사방 검사
                const X = p+x[j];
                const Y = q+y[j];
                if(X>-1 && X<N && Y>-1 && Y<M) {
                    if(A[X][Y] === 1 && !(X===0 && Y===0)) { //새로운 갈 길 체크이기 때문에 1 체크 (처음은 거리 무조건 1이니까 다시 되돌아오는 것 방지 위해 조건 추가)
                        needVisit.push([X,Y]);
                        A[X][Y] = A[p][q]+1; //앞으로 갈 곳에 이전 노드 기준 +1 거리 할당
                    }                         
                }
            }
        }
